...
const (
	...
	mInSm     = 0.01  // количество метров в одном сантиметре
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	scaledCaloriesWeight := walkingCaloriesWeightMultiplier * weight
	scaledSpeedWeight := walkingSpeedHeightMultiplier * weight
	heightM := height * mInSm
	meanSpeedMsec := meanSpeed(action, duration) * kmhInMsec
	squareOfMeanSpeed := math.Pow(meanSpeedMsec, 2)
	spentCalories := ((scaledCaloriesWeight + (squareOfMeanSpeed/heightM)*scaledSpeedWeight) * duration * minInH)
	return spentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	spentCalories := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
	return spentCalories
}
